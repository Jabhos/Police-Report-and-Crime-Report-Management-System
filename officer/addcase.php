...
<div class="container-fluid">
	...
	if(isset($_GET['pid'])){
		if($_GET['pid']!=1){
			$sq = mysqli_query($dbc, "UPDATE prev_complain SET status='1' WHERE id='".$_GET['pid']."'");
		}
		else{
			$sq = mysqli_query($dbc, "UPDATE prev_complain SET status='0' WHERE id='".$_GET['pid']."'");
		}

		$data = mysqli_fetch_array(mysqli_query($dbcon, "SELECT `statement` FROM prev_complain where id='".$_GET['pid']."'"));
		$stmt = $data['statement'];
	}
	else{
		$stmt = "Write statement Here.";
	}
					...
					<form class="form-horizontal" id="addaction"  role="form">
					  	...
					  	<div class="col-md-6">
					  		<div class="form-group">
					    		...
					    		<textarea name="statement" class="form-control"><?php echo $stmt ?></textarea>
					    		 <input type="hidden" name="staffid" value="<?php echo $staffid?>">
			...
			$.ajax({
					...
					success: function(response){
						...
						if(response.error){
							...
							setTimeout( function(){
								window.location='addcompl.php?id=1';
							}, 900);
