              ...
              $age=$_POST['age'];
               ...
               $pid = $_POST['pid'];
                ...
                if($errors){
                   ...
                   if($pid != 1){
                    $sql1 = "UPDATE prev_complain SET status='1' WHERE id='$pid'";
                    mysqli_query($dbc,$sql1);
                   }

                         if( $success and mysqli_query($dbc, $sql1))

                         {
                             
                            // header( 'Location: union_thank.php?message=success' );
                            echo "<script>alert('input')</script>";
                             $output['url'] = "addcase.php?id=staff&caseid=$trans_id&crimetype=$crime&s=$stmt&pid=$pid";
                        }
               }
