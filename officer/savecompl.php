               ...
               $pid = $_POST['pid'];
                ...
                if($errors){
                      ...
                      $output = array('error' => true, $errors);
                         ...
                         $sql = "INSERT INTO complainant (case_id, comp_name, tel, region, district,loc, addrs, age,occupation,gender,statement)
                                       VALUES(?,?,?,?,?,?,?,?,?,?,?); ";

                   $success = $db->insertRow($sql, [$trans_id, $name, $tel, $region, $district, $loc, $addrs, $age, $occ, $gender, $stmtt]);
...

                         {
                            ...
                            // header( 'Location: union_thank.php?message=success' );
                             $output['url'] = "addcase.php?id=staff&caseid=$trans_id&crimetype=$crime&pid=$pid";
                        }
               }
